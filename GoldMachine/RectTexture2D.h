...
protected://상속 받은 클래스에서만 사용가능하게
	...
	HRESULT SetTexture(ID3D11Device* pDevice, const wchar_t* filePath[], const unsigned int& texCounts)
	{
		HRESULT result;

		m_textures = new TextureResource[texCounts];
		m_textureCount = texCounts;

		for (UINT i = 0; i < texCounts; i++)
		{
			result = m_textures[i].Initialize(pDevice, filePath[i], -1);
			if (FAILED(result))
			{
				return result;
			}
