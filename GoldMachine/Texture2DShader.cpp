...
bool Texture2DShader::Initialize(ID3D11Device* pDevice, HWND hwnd)
{
	HRESULT result;
	...
	const wchar_t* vsFilename = L"../data/HLSL/TextureVS.hlsl";
	const wchar_t* psFilename = L"../data/HLSL/TexturePS.hlsl";
...
bool Texture2DShader::Render(ID3D11DeviceContext* pDeviceContext, const XMMATRIX& worldMatrix, const XMMATRIX& viewMatrix, const XMMATRIX& projectionMatrix)
{
	bool result;
...
//에러 내용을 메시지 박스로 출력
void Texture2DShader::OutputShaderErrorMessage(ID3DBlob* errorMessage, HWND hwnd)
{
	char* compileErrors;
...
//쉐이더에서 사용하는 버퍼를 업데이트
bool Texture2DShader::UpdateShaderBuffers(ID3D11DeviceContext* pDeviceContext,
	XMMATRIX worldMatrix, XMMATRIX viewMatrix, XMMATRIX projectionMatrix)
{
	...
	//정점 쉐이더의 상수 버퍼를 업데이트
	pDeviceContext->VSSetConstantBuffers(startNum, bufferCount, &m_matrixBuffer);
