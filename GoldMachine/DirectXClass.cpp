...
bool DirectXClass::Initialize(HWND hwnd)
{
	HRESULT result;
...
IDXGISwapChain* DirectXClass::GetSwapChain()
{
	return m_swapChain;
}
...
void DirectXClass::GetVideoCardInfo(char* description, int& memory)
{
...
void DirectXClass::EnableAlphaBlending()
{
	float blendFactor[4];
...
void DirectXClass::DisableAlphaBlending()
{
	float blendFactor[4];
...
void DirectXClass::EnableWireframe()
{
	//와이어 프레임 모드 적용
...
void DirectXClass::DisableWireframe()
{
	//와이어 프레임 모드 해제
	m_deviceContext->RSSetState(m_rasterState);
